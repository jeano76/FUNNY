...
// 루트 quiz.js의 나머지 로직
const baseQuizLogic = fs.readFileSync('./js/quiz.js', 'utf-8').split('const quizData = [')[0];
const quizJsLogic = fs.readFileSync('./js/quiz.js', 'utf-8').split('];')[1]; // ] 이후의 모든 로직

// 언어 목록
const languages = ['en', 'ja', 'zh', 'es', 'de', 'fr', 'ru', 'pt', 'id', 'hi', 'vi', 'th', 'tr', 'it', 'nl', 'ar', 'mn', 'la'];

// 각 언어별 quizQuestions 추출 함수
function extractQuizQuestions(langCode) {
  // 정규식으로 해당 언어의 quizQuestions 추출
  ...
  // quizQuestions 섹션 추출
  const qPattern = /quizQuestions:\s*\{([\s\S]*?)\n\s*\},/;
  const qMatch = langContent.match(qPattern);

  if (!qMatch) {
    console.log(`⚠️ ${langCode}: quizQuestions 없음`);
    return null;
  }
...
languages.forEach(lang => {
  ...
  // quizQuestions 추출
  const quizQuestionsContent = extractQuizQuestions(lang);

  if (!quizQuestionsContent) {
    console.log(`⚠️ ${lang}: quizQuestions 추출 실패`);
    return;
  }
...
`;
  ...
  const outputPath = path.join(jsFolder, 'quiz.js');
  fs.writeFileSync(outputPath, quizContent);
  console.log(`✅ ${lang.padEnd(5)} → ${langFolder}/js/quiz.js`);
});

console.log('\n✅ 모든 언어 폴더의 quiz.js 생성 완료!');
